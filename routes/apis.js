...
router.get('/userinfo', ensureLoggedIn('/auth'), ensureTokenValid, function (req, res, next) {
  ...
  handleDelivery(res, `https://${process.env.AUTH0_DOMAIN}/userinfo`, req.session.access_token);
  else handleDelivery(res, `https://${process.env.AUTH0_US_DOMAIN}/userinfo`, req.session.access_token);
});
